  ...
  return (
    <PageTransition className="p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight font-display">Municipal Departments</h1>
          <p className="text-white/40 text-sm mt-0.5 font-sans">Authoritative municipal operational divisions and zone jurisdiction</p>
        </div>
        <motion.button
          {...buttonGestures}
      ...
      {loading ? (
        ...
        <div className="card text-center py-16 bg-[#111] border-white/10 rounded-3xl space-y-4 shadow-xl">
          ...
          <p className="text-xl font-bold text-white font-display">Unable to Load Department Hierarchy</p>
        ...
        <div className="card text-center py-16 bg-[#111] border-white/10 rounded-3xl space-y-2">
          ...
          <p className="text-white font-bold font-display">No Departments Configured</p>
          <p className="text-white/40 text-xs font-mono">Municipal department hierarchy will be auto-seeded from database.</p>
        </div>
      ) : (
        <StaggerContainer className="grid sm:grid-cols-2 gap-4">
          {depts.map((d) => (
            <StaggerItem key={d.id}>
              <motion.div {...cardGestures} className="card hover:border-white/15 transition-colors h-full">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="font-black text-white leading-tight font-display">{d.name}</h3>
                    <p className="text-xs text-white/40 mt-0.5 font-mono">{d.shortName || (d as any).short_name || d.name}</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {d.categories.map((cat) => (
                    <span key={cat} className="text-xs bg-white/5 border border-white/8 text-white/50 px-2.5 py-1 rounded-full font-mono">{cat}</span>
                  ))}
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-white/50">
                    <Users className="w-4 h-4 text-[#FFC400] flex-shrink-0" />
                    <span><strong className="text-white font-display">{d.head}</strong> · Head</span>
                  </div>
                  <div className="flex items-center gap-2 text-white/50 font-mono">
                    <Phone className="w-4 h-4 text-[#FFC400] flex-shrink-0" />
                    <span>{d.contact}</span>
                  </div>
                  <div className="flex items-start gap-2 text-white/50 font-mono">
                    <MapPin className="w-4 h-4 text-[#E10600] mt-0.5 flex-shrink-0" />
                    <span>{d.zones.join(', ')}</span>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-white/8">
                  <p className="text-xs font-bold text-white/30 mb-2 uppercase tracking-wider font-mono">Teams</p>
                  <div className="flex flex-wrap gap-1.5">
                    {d.teams.map((t) => (
                      <span key={t} className="text-xs px-2.5 py-1 rounded-full bg-[#E10600]/10 border border-[#E10600]/15 text-[#E10600] font-semibold font-mono">{t}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
