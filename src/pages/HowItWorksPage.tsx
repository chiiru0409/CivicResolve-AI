...
const HowItWorksPage: React.FC = () => (
  <div className="min-h-screen bg-civic-black pt-20 pb-16">
    <div className="max-w-4xl mx-auto px-4 sm:px-6">

      {/* Header */}
      <div className="text-center mb-14">
        <div className="inline-flex items-center gap-2 bg-civic-red/10 border border-civic-red/30 text-civic-red text-sm font-bold px-4 py-2 rounded-full mb-5">
          <Brain className="w-4 h-4" />
          AI Architecture
        </div>
        <h1 className="text-4xl sm:text-5xl font-black text-civic-text mb-4">
          How <span className="text-civic-red">CivicResolve</span> AI Works
        </h1>
        <p className="text-lg text-civic-muted max-w-2xl mx-auto">
      ...
      <div className="space-y-2 mb-16">
        {pipeline.map((step, i) => (
          <div key={step.step}>
            <div className={`flex gap-4 items-start p-5 rounded-2xl border ${step.color} transition-all duration-150 hover:border-white/20`}>
              <div className="flex-shrink-0 flex flex-col items-center gap-1">
                ...
                <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full border ${step.badge}`}>{step.step}</span>
              </div>
              <div className="flex-1">
                <h3 className="font-black text-civic-text text-base">{step.label}</h3>
                <p className="text-civic-muted text-sm mt-1 leading-relaxed">{step.desc}</p>
              </div>
            </div>
            {i < pipeline.length - 1 && (
      ...
      <div className="card mb-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-xl font-black text-civic-text">Current Tech Stack (MVP)</h2>
            <p className="text-sm text-civic-muted">Production-ready architecture with simulated AI services</p>
          </div>
        </div>
        <div className="grid sm:grid-cols-2 gap-3">
          {techStack.map((item) => (
            <div key={item.label} className="flex items-center gap-3 p-3 rounded-xl bg-civic-elevated border border-civic-border">
      ...
      {/* Future integrations */}
      <div className="relative bg-civic-surface border border-civic-red/20 rounded-2xl p-8 overflow-hidden">
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-civic-red to-transparent" />
        <div className="absolute inset-0 speed-lines-bg opacity-30" />
        <div className="relative">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-black text-civic-text">Production Integrations</h2>
              <p className="text-sm text-civic-muted">Ready for real API connections</p>
            </div>
          </div>
          <div className="grid sm:grid-cols-2 gap-3">
            {futureIntegrations.map((item) => (
              <div key={item.name} className="flex items-start gap-3 bg-civic-elevated/50 hover:bg-civic-elevated rounded-xl p-4 transition-colors border border-civic-border">
                ...
                <div>
                  <p className="font-bold text-civic-text text-sm">{item.name}</p>
                  <p className="text-civic-muted text-xs mt-0.5">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
